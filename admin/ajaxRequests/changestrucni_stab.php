    ...
    redirect();
      ...
      if($ime === '' || $prezime === '' || $pozicija === ''){
        echo "Morate popuniti sva polja validnim tekstom";
           ...
           $stmtUpdate = mysqli_prepare($connection, "UPDATE strucni_stab SET ime=?, prezime=?, pozicija=? WHERE id=?");
          ...
          if(!extension($fotografija)){
           ...
           $stmtUpdate = mysqli_prepare($connection, "UPDATE strucni_stab SET ime=?, prezime=?, pozicija=?, fotografija=? WHERE id=?");
