    ...
    redirect();    
      ...
      if($ime === '' || $prezime === '' || $pozicija === ''){
        echo "Morate popuniti sva polja";
           ...
           $stmtUpdate = mysqli_prepare($connection, "UPDATE menadzment SET ime=?, prezime=?, pozicija=? WHERE id=?");
          ...
          if(!extension($fotografija)){
           ...
           $stmtUpdate = mysqli_prepare($connection, "UPDATE menadzment SET ime=?, prezime=?, pozicija=?, fotografija=? WHERE id=?");
