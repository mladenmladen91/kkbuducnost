<?php
    ...
    include "../../includes/functions.php";
     ...
     if($domacin_kosevi < 0 || $gost_kosevi < 0){
      ...
      $stmtUpdate = mysqli_prepare($connection, "UPDATE rezultati SET domacin=?, gost=?, domacin_kosevi=?, gost_kosevi=?, datum=?, vrijeme=?, liga_id =?, domacin_logo=?, gost_logo=?, dvorana=?  WHERE id=?");
      $stmtUpdate->bind_param('ssiississsi', $domacin, $gost, $domacin_kosevi, $gost_kosevi, $datum, $vrijeme, $liga_id , $domacin_logo, $gost_logo, $dvorana, $id);
      $stmtUpdate->execute();
