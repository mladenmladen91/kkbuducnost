<?php
    ...
    include "../../includes/functions.php";
      ...
      if($_FILES['fotografija']['name'] === ''){
           $stmtUpdate = mysqli_prepare($connection, "UPDATE upravni_odbor SET ime=?, prezime=?, pozicija=? WHERE id=?");
           $stmtUpdate->bind_param('sssi', $ime, $prezime, $pozicija, $id);
           $stmtUpdate->execute();
           testQuery($stmtUpdate);
           
      }else{
           $stmtUpdate = mysqli_prepare($connection, "UPDATE upravni_odbor SET ime=?, prezime=?, pozicija=?, fotografija=? WHERE id=?");
