    ...
    redirect();
      ...
      if($broj < 1 || $broj > 99){
        ...
        echo "Morate popuniti sva polja";
           ...
           $stmtUpdate = mysqli_prepare($connection, "UPDATE kadeti SET ime=?, prezime=?, broj=? WHERE id=?");
          ...
          if(!extension($fotografija)){
           ...
           $stmtUpdate = mysqli_prepare($connection, "UPDATE kadeti SET ime=?, prezime=?, fotografija=?, broj=? WHERE id=?");
